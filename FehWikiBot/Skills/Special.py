...
class Special(Skills):
    def Infobox(self):
        obj = {
            ...
            'required': ';'.join([Special.get(s).name for s in self.data['prev_id'] if s]) if self.data['prev_id'][0] else '-',
            'next': Special.get(self.data['next_id']).name if self.data['next_id'] else None,
        ...
        return super().Infobox('Special', obj).replace(' Infobox','',1)
