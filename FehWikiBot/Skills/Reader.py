...
class SkillReader(IReader):
    ...
    def parse(self):
        ...
        for _ in range(nb):
            ...
            self.skip(0x04)# self.assertBytes(4, 0xC49E0D1E, f"[{self._stack[-1][0]['id_tag']}]._2")
            self.skip(0x04)# self.assertBytes(4, 0x3128991D, f"[{self._stack[-1][0]['id_tag']}]._3")
            self.skip(0x04)# self.assertBytes(4, 0x63B8544C, f"[{self._stack[-1][0]['id_tag']}]._4")
            ...
            self.skip(0x04)# self.assertBytes(4, 0xC40160AE, f"[{self._stack[-1][0]['id_tag']}]._5")
            self.readInt('_unknow1', 0x12C0C606, signed=True)
            ...
            self.skip(0x01)# self.assertBytes(1, 0x01, f"[{self._stack[-1][0]['id_tag']}]._6")
            self.readBool('savior', 0x3C)
            self.skip(0x09)# self.assertBytes(9, 0x6444DAF4CF9DD0273C, f"[{self._stack[-1][0]['id_tag']}].end")
            self.end()
        self.end()
        return
